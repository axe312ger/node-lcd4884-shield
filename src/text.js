...
let pcd8544_buffer = [...Array((LCDHEIGHT * LCDWIDTH) / 8)].map((i) => 0)

function my_setpixel (x, y, color) {
  ...
  // x is which column
  if (color) {
    pcd8544_buffer[x + Math.floor(y / 8) * LCDWIDTH] |= _BV(y % 8)
  } else {
    pcd8544_buffer[x + Math.floor(y / 8) * LCDWIDTH] &= ~_BV(y % 8)
  }
}

module.exports = function LCDdrawstring (x, y, str) {
  cursor_x = x
  cursor_y = y

  const chars = str.split('')
  chars.forEach(LCDwrite)

  const clean = pcd8544_buffer.filter((v) => v !== 0)

  return pcd8544_buffer
}

function LCDdrawchar (x, y, s) {
...
// source: http://blog.mclemon.io/arduino-5x8-iso-8859-2-font
const font = [
